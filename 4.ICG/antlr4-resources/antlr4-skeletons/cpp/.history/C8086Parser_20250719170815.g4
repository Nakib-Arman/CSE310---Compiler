...
declaration_list 
	...
	| ID LTHIRD CONST_INT RTHIRD {
		if(symbolTable->getCurrentScopeID() == "1"){
			symbolTable->insert($ID->getText(),datatype,"global",NULL,true,$CONST_INT->getText());
			write("\t"+$ID->getText()+" DW "+$CONST_INT->getText()+" DUP (0000H)");
		}
		else {
			symbolTable->insert($ID->getText(),datatype,"local",NULL,true,$CONST_INT->getText());
			curr_func->incStackElements();
...
variable returns [string name,string type,int stack]
	: ID {
		if(symbolTable->getVariableScope($ID->getText()) == "global"){
			$name = $ID->getText();
		}
