...
public:
    ...
    pair<string,pair<int,int>> insert(string name, string type, string scope, Function* func, bool isArray=false, int array_size=0)
    {
        pair<int,int> inserted = current_scope->insert(name, type, scope, func, isArray, array_size);
    ...
    // Adding new functionalities for code generation
    ...
    {
        SymbolInfo* temp = LookUp(name);
        return temp->getStackIndex();
    }

    int getParamNum()
    {
        return current_scope->getParamNum();
    }
};