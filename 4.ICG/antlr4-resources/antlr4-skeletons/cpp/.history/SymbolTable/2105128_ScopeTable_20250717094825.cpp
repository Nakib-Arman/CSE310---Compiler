...
public:
    ...
    pair<int,int> insert(string name, string type, string scope, Function* func)
    {
    ...
    string getVariableScope(string name)
    {
        SymbolInfo* symbolInfo = LookUp(name);
        cout<<"hi"<<endl;
        if(symbolInfo == NULL) cout<<"hi2"<<endl;
        return symbolInfo->getScope();
    }

    int getStackIndex(string name)
    {
        SymbolInfo* SymbolInfo = LookUp(name);
        return SymbolInfo->getStackIndex();
    }
};
