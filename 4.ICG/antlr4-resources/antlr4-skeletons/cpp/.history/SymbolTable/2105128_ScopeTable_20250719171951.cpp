...
class ScopeTable
{
    int num_buckets, stack_index, neg_stack_index;
    string id;
...
public:
    ...
    pair<int,int> insert(string name, string type, string scope, Function* func, bool isArray=false, int array_size=0)
    {
        ...
        if(scope == "global"){
            newSymbol = new SymbolInfo(name, type, scope, 0, func);
        }
            ...
            if(!isArray){
                ...
                neg_stack_index-=array_size;
            }
            newSymbol = new SymbolInfo(name, type, scope, neg_stack_index, func);
        }
        else{
            if(!isArray){
                stack_index++;
                cout<<name<<" index "<<stack_index<<endl;
            }
            else{
                stack_index+=array_size;
                cout<<name<<" index "<<stack_index;
            }
            newSymbol = new SymbolInfo(name, type, scope, stack_index, func);
        }
        SymbolInfo *curr = hash_table[index];
        int position = 0;
        if (curr == NULL)
    ...
    // Adding new functions for code generation

    int getParamNum()
    {
        return -neg_stack_index;
    }
