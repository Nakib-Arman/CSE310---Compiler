...
func_definition 
	: return_type fn=func_name LPAREN parameter_list RPAREN compound_statement {
		if(in_main == 1){
			write("L"+to_string($fn.end_label)+":");
	...
	| return_type fn=func_name LPAREN RPAREN compound_statement {
		// symbolTable->print_all_scope_nonempty_indices();
		if(in_main == 1){
			write("L"+to_string($fn.en)+":");
			label_count++;
			write("\tADD SP, "+to_string(curr_func->getStackElements()*2));
			write("\tPOP BP\n\tMOV AX, 4CH\n\tINT 21H");
			write(curr_func->getName()+" ENDP");
...
func_name returns [int end_label]
	: ID {
