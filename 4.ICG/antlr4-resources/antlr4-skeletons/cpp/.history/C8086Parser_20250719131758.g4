...
logic_expression 
	...
	| rel_expression LOGICOP {
		int first_true_label, first_false_label, second_true_label, second_false_label,end_label;
		write("\tCMP AX, 0");
		if($LOGICOP->getText() == "||"){
			first_false_label = label_count++;
			first_true_label = label_count;
			second_true_label = label_count++;
			second_false_label = label_count++;
			end_label = second_false_label;
			write("\tJNE L"+to_string(first_true_label));
		}
		else{
			write("\tJE L"+to_string(label_count+2));
		}
	} rel_expression {
		if($LOGICOP->getText() == "||"){
			write("L"+to_string(first_false_label)+":");
			write("\tCMP AX, 0");
			write("\tJNE L"+to_string(second_true_label));
			write("\tJMP L"+to_string(second_false_label));
		}
		else{
			write("\tCMP AX, 0");
		...
		write("L"+to_string(first_true_label)+":");
		write("\tMOV AX, 1\n\tJMP L"+to_string(second_false_label));
		write("L"+to_string(second_false_label)+":");
		label_count++;
		write("\tMOV AX, 0");
		write("L"+to_string(label_count)+":");
		label_count++;
