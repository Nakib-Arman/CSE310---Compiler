...
var_declaration 
    : ts=type_specifier_stack dl=declaration_list sm=SEMICOLON {
		write("\t\t\t; Line "+to_string($SEMICOLON->getLine()));
	}
    ;
...
declaration_list 
	: declaration_list COMMA ID {
		if(symbolTable->getCurrentScopeID() == "1"){
			...
			write("\tSUB SP, 2\t"+to_string($ID->getLine()));
		}
	}
	| declaration_list COMMA ID LTHIRD CONST_INT RTHIRD {
		if(symbolTable->getCurrentScopeID() == "1"){
			...
			curr_func->incStackElements();
			write("\tSUB SP, "+to_string(stoi($CONST_INT->getText())*2)+"\t"+to_string($ID->getLine()));
		}
	}
	| ID {
		if(symbolTable->getCurrentScopeID() == "1"){
...
statement 
	...
	| PRINTLN LPAREN ID RPAREN SEMICOLON {
		...
		write("\tCALL print_output\n\tCALL new_line"+to_string($SEMICOLON->getLine()));
		write("\t\t\t; Line "+to_string($SEMICOLON->getLine()));
	}
	| {write("");} RETURN expression SEMICOLON {
		write("\tJMP L"+to_string(end_label)+"\t"+to_string($SEMICOLON->getLine()));
		write("\t\t\t; Line "+to_string($SEMICOLON->getLine()));
	}
	;
