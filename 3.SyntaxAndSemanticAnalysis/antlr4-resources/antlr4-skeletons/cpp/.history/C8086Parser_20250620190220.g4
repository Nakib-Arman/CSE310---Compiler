...
statement returns [string st_text]
	  ...
	  | IF LPAREN ex=expression RPAREN st1=statement ELSE st2=statement {
		$st_text = $IF->getText() + " " + $LPAREN->getText() + $ex.expr_text + $RPAREN->getText() + $st1.st_text + $ELSE->getText() + "\n" + $st2.st_text;
	  ...
	  | PRINTLN LPAREN ID RPAREN SEMICOLON {
		// $st_text = $PRINTLN->getText() + " " + $LPAREN->getText() + $ex.expr_text + $RPAREN->getText() + $st.st_text;
		writeIntoparserLogFile("Line " + to_string($SEMICOLON->getLine()) + ": statement : PRINTLN LPAREN expression RPAREN statement\n");
		// writeIntoparserLogFile($st_text + "\n");
	  }
