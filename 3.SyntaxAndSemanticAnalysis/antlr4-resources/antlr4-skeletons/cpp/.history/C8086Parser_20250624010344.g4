...
parameter_list returns [string param_text,string type]
		...
		| ts=type_specifier op=operator {
			$param_text = $ts.name_line;
			writeIntoparserLogFile("Line " + to_string($ts.stop->getLine()) + ": parameter_list : type_specifier\n");
			writeIntoparserLogFile($param_text + "\n");
			writeIntoparserLogFile("Error at line " + to_string($ts.stop->getLine()) + ": syntax error, unexpected " + $op.op_text + ", expecting RPAREN or COMMA\n");
			err_count++;
			$type = "error";
		}
		|
...
operator returns [string op_text]
		: ADDOP {
			$op_text = "ADDOP";
		}
		| MULOP {
