...
parameter_list returns [string param_text,string type]
		...
		| ts=type_specifier ADDOP {
			...
			writeIntoparserLogFile("Line " + to_string($ts.stop->getLine()) + ": parameter_list : type_specifier\n");
			writeIntoparserLogFile($param_text + "\n");
			writeIntoparserLogFile("Error at line " + to_string($ts.stop->getLine()) + ": syntax error, unexpected ADDOP, expecting RPAREN or COMMA\n");
			err_count++;
			$type = "error";
		}
		| ts=type_specifier MULOP {
			$param_text = $ts.name_line;
			writeIntoparserLogFile("Line " + to_string($ts.stop->getLine()) + ": parameter_list : type_specifier\n");
			writeIntoparserLogFile($param_text + "\n");
			writeIntoparserLogFile("Error at line " + to_string($ts.stop->getLine()) + ": syntax error, unexpected MULOP, expecting RPAREN or COMMA\n");
			err_count++;
			$type = "error";
		}
		| ts=type_specifier ASSIGNOP {
			$param_text = $ts.name_line;
			writeIntoparserLogFile("Line " + to_string($ts.stop->getLine()) + ": parameter_list : type_specifier\n");
			writeIntoparserLogFile($param_text + "\n");
			writeIntoparserLogFile("Error at line " + to_string($ts.stop->getLine()) + ": syntax error, unexpected ASSIGNOP, expecting RPAREN or COMMA\n");
			err_count++;
			$type = "error";
		}
		|
...
declaration_list returns [string dl_list]
		: dl=declaration_list COMMA ID {
		  ...
		  | i1=ID ADDOP i2=ID{
			bool inserted = symbolTable->insert($i1->getText(),data_type);
			if(!inserted) {
				writeIntoparserLogFile("Error at line " + to_string($i1->getLine()) + ": Multiple declaration of " + $i1->getText() + "\n");
				err_count++;
			}
			$dl_list = $i1->getText();
			writeIntoparserLogFile("Line " + to_string($i2->getLine()) + ": declaration_list : ID\n");
			writeIntoparserLogFile($dl_list + "\n");
			writeIntoparserLogFile("Error at line " + to_string($i2->getLine()) + ": syntax error, unexpected ADDOP, expecting COMMA or SEMICOLON\n");
...
variable returns [string var_text]
 ...
 expression returns [string expr_text]
	   ...
	   | var=variable ASSIGNOP sim=simple_expression ASSIGNOP {
		$expr_text = "";
		writeIntoparserLogFile("Error at line " + $ASSIGNOP->getText() + ": syntax error, unexpected ASSIGNOP\n");
	   }
	   ;
...
errorStatement
    : HASH {
        writeIntoparserLogFile("Syntax error: '#' is not allowed at line " + 
                           $HASH.getLine() + ", position " + 
                           $HASH.getCharPositionInLine());
    }
    ;
