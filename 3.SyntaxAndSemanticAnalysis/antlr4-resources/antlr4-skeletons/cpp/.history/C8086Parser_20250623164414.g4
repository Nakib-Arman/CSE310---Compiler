...
func_declaration returns [string func_dec_text]
		: ts=type_specifier fn=func_name LPAREN pm=parameter_list RPAREN SEMICOLON {
			...
			SymbolInfo* temp = symbolTable->LookUp($fn.func_name_text);
			temp->setReturnType($ts.name_line);
			temp->declareFunc();
			temp_func->clearParamList();
		}
		| ts=type_specifier fn=func_name LPAREN RPAREN SEMICOLON {
			$func_dec_text = $ts.name_line + " " + $fn.func_name_text + $LPAREN->getText() + $RPAREN->getText() + $SEMICOLON->getText();
...
func_name returns [string func_name_text]	
		: ID {
			temp_func->setName($ID->getText());
			symbolTable->insert($ID->getText(),"func",temp_func);
...
parameter_list returns [string param_text]
		: pm=parameter_list COMMA ts=type_specifier ID {
			...
			temp_func->addToParamList($ts.name_line);
...
variable returns [string var_text]
 ...
 expression returns [string expr_text]
 	   : logex=logic_expression {
		 ...
		 writeIntoparserLogFile("Line " + to_string($logex.stop->getLine()) + ": expression : logicexpression\n");
