...
func_definition returns [string func_def_text] 
		: ts=return_type fn=func_name LPAREN pm=parameter_list rp=r_paren cmst=compound_statement {
			if($cmst.type == "return" && $ts.name_line == "void") {
				writeIntoparserLogFile("Error at line " + to_string($cmst.stop->getLine()) + ": Cannot return value from function " + $fn.func_name_text + " with void return type\n");
				writeIntoparserErrorFile("Error at line " + to_string($cmst.stop->getLine()) + ": Cannot return value from function " + $fn.func_name_text + " with void return type\n");
				err_count++;
			}
		...
		: pm=parameter_list COMMA ts=type_specifier ID {
			if(!in_declaration){
				...
				if (!inserted) {
					...
					writeIntoparserErrorFile("Error at line " + to_string($ID->getLine()) + ": Multiple declaration of " + $ID->getText() + " in parameter\n");
					err_count++;
				}
			}
 		...
 		| ts=type_specifier ID {
			...
			if(!in_declaration){
				...
				if (!inserted) {
					...
					writeIntoparserErrorFile("Error at line " + to_string($ID->getLine()) + ": Multiple declaration of " + $ID->getText() + "\n");
					err_count++;
				}
			}
			cur_func->addToParamList($ts.name_line);
...
compound_statement returns [string cmst_text,string type]
...
var_declaration returns [string var_text]
    : t=type_specifier dl=declaration_list sm=SEMICOLON {
		...
		if ($t.name_line == "void") {
			...
			writeIntoparserErrorFile("Error at line " + to_string($sm->getLine()) + ": Variable type cannot be void\n");
			err_count++;
		}
...
statements returns [string stmts_text,string type]
	   : st=statement {
		$type = $st.type;
...
statement returns [string st_text,string type]
	  : vd=var_declaration {
		$st_text = $vd.var_text;
	  ...
	  | FORIN LPAREN v=variable IN cs1=CONST_INT TO cs2=CONST_INT RPAREN st=statement {
		$st_text = $FORIN->getText() + $LPAREN->getText() + $v.var_text + $IN->getText() + $cs1->getText() + $TO->getText() + $cs2->getText() + $RPAREN->getText() + $st.st_text;
		writeIntoparserLogFile("Line " + to_string($st.stop->getLine()) + ": statement : FOR LPAREN variable IN CONST_INT TO CONST_INT RPAREN statement\n");
		writeIntoparserLogFile($st_text + "\n");
		if($v.type != "int"){
			writeIntoparserLogFile("Error\n");
		}
	  }
	  ...
	  | PRINTLN LPAREN ID RPAREN SEMICOLON {
		...
		if (temp == NULL) {
			...
			writeIntoparserErrorFile("Error at line " + to_string($SEMICOLON->getLine()) + ": Undeclared variable " + $ID->getText() + "\n");
			err_count++;
		}
		writeIntoparserLogFile($st_text + "\n");
	  }
...
expression_statement returns [string expr_stmt_text]	
			...
			| exp=expression {
				$expr_stmt_text = $exp.expr_text ;
				writeIntoparserLogFile("Line : expression_statement : expression SEMICOLON\n");
				writeIntoparserLogFile($expr_stmt_text + "\n");
			}
			;
	  
variable returns [string var_text,string type]
	 : ID {
		...
		if(data_type == "") {
			...
			writeIntoparserErrorFile("Error at line " + to_string($ID->getLine()) + ": Undeclared variable " + $ID->getText() + "\n");
			err_count++;
		}
		else if (suffix == " array") {
			writeIntoparserLogFile("Error at line " + to_string($ID->getLine()) + ": Type mismatch, " + $ID->getText() + " is an array\n");
			writeIntoparserErrorFile("Error at line " + to_string($ID->getLine()) + ": Type mismatch, " + $ID->getText() + " is an array\n");
			err_count++;
		}
	 ...
	 | ID LTHIRD ex=expression RTHIRD {
		...
		if (suffix != " array") {
			...
			writeIntoparserErrorFile("Error at line " + to_string($RTHIRD->getLine()) + ": " + $ID->getText() + " not an array\n");
			err_count++;
		}
		if ($ex.type != "int") {
			writeIntoparserLogFile("Error at line " + to_string($RTHIRD->getLine()) + ": Expression inside third brackets not an integer\n");
			writeIntoparserErrorFile("Error at line " + to_string($RTHIRD->getLine()) + ": Expression inside third brackets not an integer\n");
			err_count++;
		}
		writeIntoparserLogFile($var_text + "\n");
 ...
 expression returns [string expr_text,string type]
	   ...
	   | var=variable ASSIGNOP ternary_expression {
	     writeIntoparserLogFile("Line " + to_string($logex.stop-\n");
	   }
	   | var=variable ASSIGNOP logex=logic_expression {
		 ...
		 data_type = $var.type;
		 string element_type = data_type.substr(0, data_type.find(' '));
		 if (data_type != "" && $logex.type != "Undefined"){
			if (element_type != $logex.type) {
				if (data_type == "void" || $logex.type == "void") {
					writeIntoparserLogFile("Error at line " + to_string($logex.stop->getLine()) + ": Void function used in expression\n");
					writeIntoparserErrorFile("Error at line " + to_string($logex.stop->getLine()) + ": Void function used in expression\n");
					err_count++;
					if (data_type != "void") $type = data_type;
					else if ($logex.type != "void") $type = $logex.type;
					else $type = "void";
				}
				else{
					if (data_type == "float" && $logex.type == "int") {
					}
					else{
						writeIntoparserLogFile("Error at line " + to_string($logex.stop->getLine()) + ": Type Mismatch\n");
						writeIntoparserErrorFile("Error at line " + to_string($logex.stop->getLine()) + ": Type Mismatch\n");
						err_count++;
					}
				}
			}
...
logic_expression returns [string logic_text,string type]
...
ternary_expression returns [string ter_text, string type]
		: LPAREN ex=expression RPAREN QMARK exst1=expression_statement COLON exst2=expression_statement {
			writeIntoparserLogFile("Line hi\n");
		}
		|
		;
			
rel_expression returns [string rel_text,string type]
...
simple_expression returns [string sim_text,string type]
		  : term {
			$type = $term.type;
		  ...
		  | smex=simple_expression ADDOP term {
			if($smex.type == "float" || $term.type == "float") $type="float";
			else $type = "int";
...
term returns [string term_text,string type]
     ...
     |  t=term MULOP unex=unary_expression {
		...
		if ($t.type != "int" || $unex.type != "int") {
			if ($MULOP->getText() == "%") {
				writeIntoparserLogFile("Error at line " + to_string($unex.stop->getLine()) + ": Non-Integer operand on modulus operator\n");
				writeIntoparserErrorFile("Error at line " + to_string($unex.stop->getLine()) + ": Non-Integer operand on modulus operator\n");
				err_count++;
				$type = "int";
			}
			else{
				if($t.type == "void" || $unex.type == "void") {
					writeIntoparserLogFile("Error at line " + to_string($unex.stop->getLine()) + ": Void function used in expression\n");
					writeIntoparserErrorFile("Error at line " + to_string($unex.stop->getLine()) + ": Void function used in expression\n");
					err_count++;
					if($t.type != "void") $type = $t.type;
					else if ($unex.type != "void") $type = $unex.type;
					else{
						$type = "void";
					}
				}
		...
		if ($MULOP->getText() == "%" && $unex.unex_text == "0") {
			...
			writeIntoparserErrorFile("Error at line " + to_string($unex.stop->getLine()) + ": Modulus by Zero\n");
			err_count++;
			$type = $t.type;
		}
		writeIntoparserLogFile($term_text + "\n");
...
unary_expression returns [string unex_text,string type]
...
factor returns [string fact_text,string type]	
	: var=variable {
	...
	| ID LPAREN al=argument_list RPAREN {
		...
		Function* temp = symbolTable->getFunc($ID->getText());
		if(temp == NULL) {
			writeIntoparserLogFile("Error at line " + to_string($RPAREN->getLine()) + ": Undefined function " + $ID->getText() + "\n");
			writeIntoparserErrorFile("Error at line " + to_string($RPAREN->getLine()) + ": Undefined function " + $ID->getText() + "\n");
			err_count++;
			$type = "Undefined";
		}
		else{
			...
			if (suffix != " array") {
				...
				if (argument_types.size() != param_list.size()){
					writeIntoparserLogFile("Error at line " + to_string($RPAREN->getLine()) + ": Total number of arguments mismatch with declaration in function " + $ID->getText() + "\n");
					writeIntoparserErrorFile("Error at line " + to_string($RPAREN->getLine()) + ": Total number of arguments mismatch with declaration in function " + $ID->getText() + "\n");
					err_count++;
				}
				else{
					for(int i=0;i<param_list.size();i++) {
						if(argument_types[i] != param_list[i]) {
							writeIntoparserLogFile("Error at line " + to_string($RPAREN->getLine()) + ": " + to_string(i+1) + "th argument mismatch in function " + $ID->getText() + "\n");
							writeIntoparserErrorFile("Error at line " + to_string($RPAREN->getLine()) + ": " + to_string(i+1) + "th argument mismatch in function " + $ID->getText() + "\n");
							err_count++;
							break;
						}
					}
...
arguments returns [string arg_text]
		  : args=arguments COMMA logex=logic_expression {
			...
			argument_types.push_back($logex.type);
		  }
	      | logex=logic_expression {
