...
func_name_dec returns [string func_name_text]	
		: ID {
			...
			symbolTable->insert($ID->getText(),"func",cur_func);
			has_param = 1;
			in_declaration = 1;
...
func_definition returns [string func_def_text] 
		: ts=type_specifier fn=func_name LPAREN pm=parameter_list rp=r_paren cmst=compound_statement {
		...
		| ts=type_specifier fn=func_name LPAREN rp=r_paren cmst=compound_statement {
			$func_def_text = $ts.name_line + " " + $fn.func_name_text + $LPAREN->getText() + $rp.r + $cmst.cmst_text;
