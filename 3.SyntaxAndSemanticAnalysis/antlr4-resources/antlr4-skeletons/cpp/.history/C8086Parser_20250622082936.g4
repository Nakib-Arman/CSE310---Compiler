...
compound_statement returns [string cmst_text]
			: curlybrace stmts=statements RCURL {
				$cmst_text = $curlybrace->getText() + "\n" + $stmts.stmts_text + $RCURL->getText();
				writeIntoparserLogFile("Line " + to_string($RCURL->getLine()) + ": compound_statement : curlybrace statements RCURL\n");
				writeIntoparserLogFile($cmst_text + "\n");
			}
 		    | curlybrace RCURL {
				$cmst_text = $curlybrace->getText() + $RCURL->getText();
				writeIntoparserLogFile("Line " + to_string($RCURL->getLine()) + ": compound_statement : curlybrace RCURL\n");
				writeIntoparserLogFile($cmst_text + "\n");
			}
 		    ;
