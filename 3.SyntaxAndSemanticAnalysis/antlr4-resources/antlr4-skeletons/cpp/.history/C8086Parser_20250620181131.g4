...
statement returns [string st_text]
	  ...
	  | IF LPAREN ex=expression RPAREN st=statement {
		$st_text = $IF->getText() + $LPAREN->getText() + $ex.expr_text + $RPAREN->getText() + $st->getText();
		writeIntoparserLogFile("Line " + to_string($st.stop->getLine()) + ": statement : IF LPAREN expression RPAREN\n");
		writeIntoparserLogFile($st_text + "\n\n");
	  }
