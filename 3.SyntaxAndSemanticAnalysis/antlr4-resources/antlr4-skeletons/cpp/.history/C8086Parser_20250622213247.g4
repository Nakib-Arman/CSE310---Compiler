...
func_declaration returns [string func_dec_text]
		: ts=type_specifier fn=func_name LPAREN pm=parameter_list RPAREN SEMICOLON {
			...
			symbolTable->insert($fn.func_name_text,"func","");
...
variable returns [string var_text]
	 ...
	 | ID LTHIRD ex=expression RTHIRD {
		...
		if (suffix != "array") {
			writeIntoparserLogFile("Error at line " + to_string($RTHIRD->getLine()) + ": " + $ID->getText() + "Expression inside third brackets not an integer\n");
			err_count++;
		}
		if (type2 != "int") {
