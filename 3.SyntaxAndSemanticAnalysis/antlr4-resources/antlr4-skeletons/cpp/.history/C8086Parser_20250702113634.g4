...
statement returns [string st_text,string type]
	  ...
	  | FORIN LPAREN v=variable IN cs1=CONST_INT TO cs2=CONST_INT RPAREN st=statement {
		$st_text = $FOR->getText() + $LPAREN->getText() + $v.var_text + $IN->getText() + $cs1->getText() + $TO->getText() + $cs2->getText() + $RPAREN->getText() + $st.st_text;
		...
		writeIntoparserLogFile($st_text + "\n");
		if(v.type != "int"){
			writeIntoErrorLogFile("Error " + to_string(RPAREN))
		}
	  }
