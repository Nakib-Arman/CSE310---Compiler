...
declaration_list returns [string dl_list]
		: dl=declaration_list COMMA ID {
 		  ...
 		  | ID LTHIRD CONST_INT RTHIRD {
			$dl_list = $ID->getText() + $LTHIRD->getText() + $CONST_INT->getText() + $RTHID->getText();
			writeIntoparserLogFile("Line " + to_string($RTHIRD->getLine()) + ": declaration_list : ID LTHIRD CONST_INT RTHI\n");
			writeIntoparserLogFile($ID->getText() + "\n");
		}
 		  ;
