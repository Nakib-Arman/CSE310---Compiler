...
unit returns [string unit_text]
	: vd=var_declaration {
     ...
     | fdef=func_definition {
		$unit_text = $fd.func_def_text;
		writeIntoparserLogFile("\nLine " + to_string($fd.stop->getLine()) + ": unit : func_definition\n");
		writeIntoparserLogFile($fd.func_def_text + "\n");
	 }
     ;
...
func_definition returns [string fun_def_text] 
		: ts=type_specifier ID LPAREN pm=parameter_list RPAREN cmst=compound_statement {
			$fun_def_text = $ts.name_line + " " + $ID->getText() + $LPAREN->getText() + $pm.param_text + $RPAREN->getText() + $cmst.cmst_text;
			...
			writeIntoparserLogFile($fun_def_text + "\n\n");
		}
...
statements returns [string stmts_text]
	   ...
	   | stmts=statements st=statement {
		$stmts_text = $stmts_text + " " + $st.st_text;
...
expression_statement returns [string expr_stmt_text]	
			: SEMICOLON {
				...
				writeIntoparserLogFile("Line " + $SEMICOLON->getText() + ": expression_statement : SEMICOLON\n");
				writeIntoparserLogFile($expr_stmt_text + "\n");
			}			
			| exp=expression SEMICOLON {
				$expr_stmt_text = $exp.expr_text + $SEMICOLON->getText();
				writeIntoparserLogFile("Line " + $SEMICOLON->getText() + ": expression_statement : expression SEMICOLON\n");
				writeIntoparserLogFile($expr_stmt_text + "\n");
			}
			;
