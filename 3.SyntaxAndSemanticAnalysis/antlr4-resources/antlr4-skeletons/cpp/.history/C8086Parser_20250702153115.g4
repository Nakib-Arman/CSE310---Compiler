...
statement returns [string st_text,string type]
	  ...
	  | PRINT LPAREN STRING RPAREN {
		$st_text = $PRINT->getText() + $LPAREN->getText() + $STRING->getText() + $RPAREN->getText();
		writeIntoparserLogFile("Line " + to_string($RPAREN>getLine()) + ": statement : PRINT LPAREN STRING RPAREN\n");
		writeIntoparserLogFile($st_text + "\n");
	  }
	  | WHEN LPAREN ex=expression RPAREN LCURL csl=case_list e=else_st LCURL {
		$st_text = $WHEN->getText() + $LPAREN->getText() + $ex.expr_text + $RPAREN->getText() + $csl.csl_text + e.s_text + LCURL->getText();
		writeIntoparserLogFile("Line " + to_string($LPAREN->getLine()) + ": statement : WHEN LPAREN expression RPAREN LCURL case_list else_st LCURL\n");
		writeIntoparserLogFile($st_text + "\n");
	  }
	  |
	  ;
...
case_list returns [string csl_text]
	: cs=case {
		cout<<"Hello4"<<endl;
	}
	| csl=case_list cs=case {
		cout<<"hi5"<<endl;
	}
	|
	;

case returns [string cs_text]
	: CONST_INT ARROW statements {
		cout<<"Hello2"<<endl;
	}
	| CONST_FLOAT ARROW statements {
		cout<<"Hello3"<<endl;
	}
	| 
	;

else_st returns [string e_text]
	: ELSE ARROW statements {
		cout<<"Hello6"<<endl;
	}
	|
	;
